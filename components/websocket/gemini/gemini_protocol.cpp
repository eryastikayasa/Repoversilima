...
bool gemini_protocol_build_setup(char **output,size_t *output_len){
    ...
    *output=nullptr;*output_len=0;
    cJSON *root=cJSON_CreateObject();if(!root)return false;cJSON *setup=cJSON_AddObjectToObject(root,"setup");
...
bool gemini_protocol_build_realtime_audio(const int16_t *pcm16,size_t samples,char **output,size_t *output_len){
    ...
    *output=nullptr;*output_len=0;
    const size_t pcm_bytes=samples*sizeof(int16_t);const size_t encoded_len=base64_encoded_size(pcm_bytes);char *encoded=static_cast<char *>(malloc(encoded_len+1U));if(!encoded)return false;if(!base64_encode(reinterpret_cast<const uint8_t *>(pcm16),pcm_bytes,encoded,encoded_len+1U)){free(encoded);return false;}
...
bool gemini_protocol_build_realtime_text(const char *text,char **output,size_t *output_len){
    ...
    *output=nullptr;*output_len=0;
    cJSON *root=cJSON_CreateObject();cJSON *realtime=root?cJSON_AddObjectToObject(root,"realtimeInput"):nullptr;if(!root||!realtime){cJSON_Delete(root);return false;}cJSON_AddStringToObject(realtime,"text",text);char *json=cJSON_PrintUnformatted(root);cJSON_Delete(root);if(!json)return false;*output=json;*output_len=strlen(json);return true;
}

bool gemini_protocol_process_message(const char *json,size_t len){
    if(!json||len==0)return false;
    cJSON *root=cJSON_ParseWithLength(json,len);if(!root){ESP_LOGW(TAG,"Pesan Gemini bukan JSON valid (%u byte)",(unsigned)len);return false;}const gemini_message_type_t type=gemini_message_classify(json,len);bool handled=type!=GEMINI_MESSAGE_UNKNOWN;
