...
static bool init_audio_pools(void) {
    ...
    ESP_LOGI(TAG,"Audio buffers: MIC=%u x %uB (~5s), SPK=%u x %uB (~4s), PSRAM",(unsigned)MIC_QUEUE_DEPTH,(unsigned)MIC_FRAME_BYTES,(unsigned)PLAYBACK_QUEUE_DEPTH,(unsigned)(PLAYBACK_BLOCK_BYTES+sizeof(uint16_t)));return true;
}
static void wakeword_task(void *) { while(s_wakeword_running){if(audio_engine_process_wakeword())ESP_LOGI(TAG,"WakeWord event diterima AudioEngine");vTaskDelay(pdMS_TO_TICKS(WAKEWORD_TASK_DELAY_MS));}s_wakeword_task=nullptr;vTaskDelete(nullptr); }
