...
static uint32_t s_playback_block_count = 0;
...
static void wakeword_task(void *) { while(s_wakeword_running){if(audio_engine_process_wakeword()){}vTaskDelay(pdMS_TO_TICKS(WAKEWORD_TASK_DELAY_MS));}s_wakeword_task=nullptr;vTaskDelete(nullptr); }
...
extern "C" bool audio_engine_init(void){if(s_initialized)return true;audio_hal_init();if(!wakeword_init())return false;if(wakeword_get_sample_rate()!=16000){wakeword_deinit();return false;}if(!init_audio_pools()){wakeword_deinit();return false;}s_wakeword_detected=false;s_wakeword_running=false;s_conversation_running=false;s_playback_running=false;s_mic_frame_count=0;s_playback_block_count=0;s_initialized=true;return true;}
...
extern "C" bool audio_engine_start_conversation(void){if(!s_initialized||s_conversation_running)return s_conversation_running;if(!stop_wakeword_and_wait())return false;if(audio_hal_start_capture()!=ESP_OK)return false;xQueueReset(s_mic_free_queue);xQueueReset(s_mic_ready_queue);for(size_t i=0;i<MIC_QUEUE_DEPTH;++i)(void)xQueueSend(s_mic_free_queue,&s_mic_buffers[i],0);s_mic_frame_count=0;s_conversation_running=true;if(xTaskCreate(conversation_task,"audio_conversation",CONVERSATION_TASK_STACK,nullptr,5,&s_conversation_task)!=pdPASS){s_conversation_running=false;audio_hal_stop_capture();s_conversation_task=nullptr;return false;}return true;}
